...
On the discovery benchmark the formula beats default gradient
boosting on 9 of 31 datasets on point estimates; under the
Nadeau--Bengio correction for repeated-CV dependence, three of
those wins remain individually resolvable: Haberman
($\Delta F_1 = {+}0.175$), Liver ($+0.159$), and Transfusion
($+0.118$).  All three survive a tuned baseline and a fully
symmetric scoring protocol.  A \emph{pre-registered confirmatory
study} on 26 fresh OpenML datasets---selection criteria, bars, and
analysis frozen before any data were scored---replicates the effect
out of sample: the formula wins the sign test (19/26,
$p = 0.014$), with ten individually significant wins up to
$\Delta F_1 = {+}0.22$.  Two pre-registered probes then bound what
the effect \emph{is}: a threshold-free AUC comparison flips sign
(mean ${-}0.017$), showing the advantage lies in operating-point
selection rather than ranking quality, and the
small-$N$/low-$d$ ``regime'' observed in the discovery set fails
to replicate ($\rho(\log_2 N, \Delta F_1) = {+}0.12$ out of
sample).  We report all results honestly, including the 19
discovery-set losses, and provide PLS loading analyses showing
that winning formulas map to domain-interpretable feature
combinations.

Code: available upon acceptance (anonymous review).
Review artifacts (pre-registrations, frozen candidate list,
fold-level results, verdict-recomputing scripts):
\url{https://github.com/ahb-sjsu/theory-radar-review}.
\end{abstract}
...
well-defined), (ii)~\emph{fair evaluation} (formulas and baselines are scored on
identical train/test splits; a residual scoring asymmetry in the
discovery protocol---threshold fitting for the formula arm
only---was identified post hoc, quantified, and removed in the
confirmatory protocol of Sec.~\ref{sec:confirmatory}), and (iii)~\emph{honest reporting} (losses are reported
...
protocol ($200 \times 5$-fold CV, three baselines,
dependence-corrected inference) on which three formula wins are
individually resolvable under the Nadeau--Bengio correction and
survive both baseline tuning and a symmetric scoring protocol.
\item A \textbf{pre-registered confirmatory study}
(Sec.~\ref{sec:confirmatory}): on 26 fresh datasets selected
blind, the formula wins the sign test ($19/26$, $p = 0.014$; ten
individually significant wins), while threshold-free scoring
flips the aggregate sign---locating the advantage in
operating-point selection, not ranking.
...
Repeated CV produces correlated fold-level measurements: the
1{,}000 paired differences share training data and are far from
independent.  Our primary inference therefore uses the
Nadeau--Bengio corrected resampled
$t$-test~\cite{nadeau2003inference}, with
$\rho = n_{\mathrm{test}}/n_{\mathrm{train}} = 1/4$ at 5-fold
CV.  The correction inflates the naive variance by a factor of
$\approx 251$ at 1{,}000 folds; a direct measurement of the
dependence structure on independent subsample replicates (reported
in the confirmatory study's companion analysis) finds the
heuristic mildly conservative, supporting its use.  Under this
correction, three of the nine point-estimate wins in
Table~\ref{tab:full} remain individually resolvable: Haberman,
Liver, and Transfusion.  The per-dataset intervals printed in
Table~\ref{tab:full} are computed from the raw paired differences
and are \emph{descriptive effect-size summaries only}; they are
roughly $16\times$ too narrow to be read as hypothesis tests, and
outcome labels in the table follow the descriptive rule for
continuity with the point estimates.  Wherever this paper asserts
statistical significance, the Nadeau--Bengio correction is in
force.
...
95\% CI from 1{,}000 paired differences (descriptive only; see
Sec.~\ref{sec:methodology}).  Outcome labels follow the
descriptive rule; under Nadeau--Bengio inference only Haberman,
Liver, and Transfusion are individually resolvable wins.}
\label{tab:full}
\scriptsize
...
\end{tikzpicture}
\caption{Win/loss scatter on the \emph{discovery} benchmark:
sample size ($N$, log scale) vs.\ dimensionality ($d$).  Formula
wins (green circles) cluster at $N \leq 800$ in this collection.
This pattern is descriptive of the discovery set only: it did not
replicate out of sample (Sec.~\ref{sec:confirmatory}), and we no
longer advance it as a regime claim.}
\label{fig:scatter}
\end{figure}
...
The nine point-estimate wins span a wide range of effect sizes,
and subsequent analysis resolved the question the margins raise.
Under Nadeau--Bengio inference, only Liver
($\Delta F_1 = +0.159$), Haberman ($+0.175$), and Transfusion
($+0.118$) are individually resolvable; the six narrow wins are
statistically indistinguishable from ties at the corrected
standard error.  A tuned-baseline experiment (random search, 20
candidates, nested on the training folds) confirmed the same
split empirically: tuning erased Heart on point estimates, reduced
Wine and Breast~Cancer to within rounding, and left the three
large-margin wins essentially untouched
($-0.000$, $+0.012$, $+0.007$ erosion respectively).  Readers
should treat the three large-margin wins as the benchmark's
defensible individual results.
...
match this representational capacity.  Within-dataset learning
curves on ten large datasets confirm a real size effect
(the formula's relative advantage falls by ${\approx}0.016\ F_1$
per doubling of $n$, $t = -4.8$ with dataset intercepts), but the
crossing point varies by two orders of magnitude across datasets,
and the between-dataset correlation did not replicate out of
sample (Sec.~\ref{sec:confirmatory}); we therefore report the
size effect as within-dataset and dataset-specific, not as a
universal threshold.
...
\subsection{Pre-Registered Confirmatory Study}
\label{sec:confirmatory}

Because the win/loss patterns above were observed in the same data
used to characterize them, we conducted a pre-registered
confirmatory study.  The registration---selection criteria,
numerical bars, and analysis plan---was committed before any
candidate dataset was scored, and the candidate list was frozen by
a separate commit before the first run.
All registrations, the frozen list, fold-level results, and scripts
that recompute every verdict in this section are public
(\url{https://github.com/ahb-sjsu/theory-radar-review}).

\textbf{Protocol symmetry.}  The discovery protocol contained a
scoring asymmetry: the formula arm's decision threshold was fitted
on the training fold, while baselines used the default 0.5 cut.
Measured on the three large wins, this asymmetry was worth
${+}0.037$ (Haberman), ${+}0.056$ (Transfusion), and ${+}0.032$
(Liver) of $\Delta F_1$.  The confirmatory protocol removes it:
both arms receive identical training-fold threshold optimization,
applied to \texttt{predict\_proba} for the baselines.  A
threshold-free ROC--AUC comparison is reported alongside as a
secondary protocol.

\textbf{Design.}  Candidates were drawn blind from OpenML: binary,
tabular, $150 \leq N \leq 800$, $d \leq 10$, at least 20
minority instances, excluding the 31 discovery datasets and their
variants and all synthetic or time-series data, sorted by ascending
OpenML id.  The pool was exhausted at 26 candidates (30 were
registered; no post-hoc replacement was permitted).  Two frozen
datasets were later found label-degenerate (\texttt{mu284} leaks
its target as a feature; \texttt{iris} is trivially separable);
per the registration they remain in the primary count and are
excluded in a declared sensitivity.  Five census near-duplicates
constitute one dataset family; a family-collapsed count was
declared alongside.  Gate: the three discovery wins re-scored
under the symmetric protocol (C0) must remain wins, else no
confirmatory result counts.

\textbf{Results.}  C0 passed: Haberman ${+}0.148$
$[{+}0.049, {+}0.246]$, Liver ${+}0.124$ $[{+}0.052, {+}0.196]$,
Transfusion ${+}0.057$ $[{+}0.000, {+}0.114]$ (Nadeau--Bengio 95\%
intervals; $\rho = 1/9$ at the registered 10-fold CV).  On the 26
fresh datasets the formula won 19 ($p = 0.014$, one-sided exact
binomial against $1/2$); excluding the two degenerate datasets,
18/24 ($p = 0.011$); collapsed to one vote per family, 12/18
($p = 0.12$)---the direction replicates, while the count's
significance depends on the independence accounting, and we report
all three declared views.  Ten candidates were individually
significant with $\Delta F_1 \geq 0.05$ (largest ${+}0.221$),
though five of the ten belong to a single census family and eight
derive from binarized regression sources---the confirmatory
population, drawn by the registered filters, consists largely of
binarized regression tasks with near-balanced classes rather than
the clinical datasets of the discovery benchmark.

\textbf{What the effect is, and is not.}  Two pre-registered
probes bound the interpretation.  First, the discovery-set size
pattern \emph{failed to replicate}:
$\rho(\log_2 N, \Delta F_1) = {+}0.12$ (wrong sign) and
$\rho(\log_2 d, \Delta F_1) = {-}0.14$ ($p = 0.50$) on the
fresh datasets.  Together with within-dataset learning curves
whose crossing points span two orders of magnitude, and a
synthetic capacity experiment in which \emph{reducing} GB depth
widened rather than narrowed the formula's advantage, we withdraw
the small-$N$/low-$d$ regime claim.  Second, under the
threshold-free AUC protocol the aggregate sign flips: mean
$\Delta F_1 = {+}0.053$ but mean $\Delta\mathrm{AUC} =
{-}0.017$.  The formula does not rank test instances better than
gradient boosting; its advantage arises in the selection of an
$F_1$-optimal operating point on near-balanced tasks, where the
formula-plus-fitted-threshold pipeline is more effective than
thresholding a boosted ensemble's probabilities.  We regard this
threshold-mediated characterization, together with the three
clinical wins whose mechanism remains open, as the accurate
statement of the method's strength.
...
The headline benchmark uses scikit-learn defaults.  This
limitation has since been quantified: a random-search tuning
experiment (20 candidates, inner 3-fold on training data only,
all 31 datasets) erased Heart on point estimates, reduced Wine
and Breast~Cancer to within rounding, and left the three
large-margin wins essentially untouched; tuning gains showed no
correlation with the win pattern
($\rho = -0.03$, $p = 0.89$), refuting the hypothesis that the
wins mark datasets where default GB is merely misconfigured.
Default results remain the headline for reproducibility; the
tuned comparison bounds their fragility.
...
\begin{itemize}
\item On the discovery benchmark, three wins are individually
resolvable under dependence-corrected inference---Haberman
($+0.175$), Liver ($+0.159$), Transfusion ($+0.118$)---and all
three survive baseline tuning and a symmetric scoring protocol.
\item A pre-registered confirmatory study on 26 fresh datasets
replicates the effect out of sample: 19/26 wins ($p = 0.014$),
ten individually significant with effects up to $+0.22$.
\item On 19 discovery datasets (61\%), ensembles win, and the
method quantifies the gap honestly.
\end{itemize}

Two pre-registered probes locate the effect precisely.  The
small-$N$/low-$d$ pattern visible in the discovery set is
withdrawn as a regime claim: it failed to replicate out of sample,
within-dataset crossing points vary by two orders of magnitude,
and reducing ensemble capacity widens rather than narrows the
formula's advantage.  Under threshold-free scoring the aggregate
sign flips ($\Delta\mathrm{AUC} = -0.017$): the method's
advantage is \emph{operating-point selection}---the
formula-plus-fitted-threshold pipeline finds better $F_1$ cuts on
near-balanced tasks---rather than superior ranking.  PLS loading
analysis confirms that winning formulas map to clinically
